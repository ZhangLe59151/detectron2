...
class Box2BoxTransform(object):
    ...
    def get_relative_areas_ratio(self, src_boxes_list, target_boxes_list, areas, pred_class_logits, gt_classes, gt_sampled_targets):
        ...
        for box in target_box:
            ...
            for item in target_boxes_list[0]:
                if box.equal(item):
                    if (not torch.gt(pred_class_logits[i][0], score_num) and torch.gt(pred_class_logits[i][0], pred_class_logits[i][1])):
                        score_num = pred_class_logits[i][0]
                        print('closed pre box score : ', score_num)
                        print('closed pre box: ', src_boxes_list[0][i])
